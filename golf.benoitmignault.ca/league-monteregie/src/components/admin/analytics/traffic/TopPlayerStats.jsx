 ...
 * 
 * @description
 * Ce composant récupère les données des joueurs les plus consultés de la section évolution des joueurs
 ...
 * @returns {JSX.Element} - Le composant TopPlayerStats.
 */
function TopPlayerStats({ selectedPeriod }) {

    // Utilisation de useNavigate pour rediriger l'utilisateur vers le bon lien en cas de session invalide
    const navigate = useNavigate();

    // État pour stocker les joueurs les plus consultés mais de la section évolution des joueurs
    const [playerStats, setPlayerStats] = useState(null);

    // État pour stocker les messages d'erreur
    const [error, setError] = useState("");

    // Fonction pour charger les joueurs les plus consultés de la section évolution des joueursdepuis l'API en fonction de la période sélectionnée
    const loadTopPlayerStatsData = async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/admin/analytics/traffic/get-top-players-stats.php?period=${selectedPeriod}`,
                {
                    credentials: "include"
                }
            );
            ...
            if (data.success) {
                ...
                setPlayerStats(data.top_players);
