<?php
    // Cette fonction sera appelée lors de la création des deux nombres.
    // Elle ne fait pas la distinction entre nombre 1 et nombre 2
    function construction_nombre(float $nombre, float $unChiffre) {
        if (empty($nombre)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['un']) || isset($_POST['deux']) || isset($_POST['trois']) ||
            ...
            switch ($typeOpe) {
                case "" : $nombre1 = construction_nombre($nombre1, $unChiffre);
                    break;
                default : $nombre2 = construction_nombre($nombre2, $unChiffre);
                    break;
            }
        }
...
<head>
    ...
    <style>
        body {
            ...
            background-image: url("calcul-background.jpg");
...
<body>
    ...
    <form method="post" action="/calculatrice/calcul.php">
