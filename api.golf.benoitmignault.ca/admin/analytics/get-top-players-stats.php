...
// Requête SQL pour récupérer les 10 joueurs les plus consultés sur le site web
$select = "SELECT p.firstname, p.lastname, count(*) as clicks ";
$from = "FROM players p LEFT JOIN website_logs wl ON p.id = wl.target_id ";
$where = "WHERE wl.action_type = 'player_stats_view' ";
...
// Initialiser un tableau pour stocker les joueurs les plus consultés
$topPlayers = [];

// Parcourir les résultats de la requête SQL et les stocker dans le tableau $topPlayers
...
echo json_encode(["success" => true, "top_players" => $topPlayers]);
